class tagSphere {
  ...
  constructor(container = document.body, texts, options) {
    ...
    self.texts = texts || []
    self.config = { ...tagSphere._defaultConfig, ...(options || {}) }
    ...
    self.maxSpeed = tagSphere._getMaxSpeed(self.config.maxSpeed) // rolling max speed
    self.initSpeed = tagSphere._getInitSpeed(self.config.initSpeed) // rolling init speed
    ...
    tagSphere.list.push({ el: self.$el, container, instance: self })
  }
  ...
  _init() {
    ...
    // mouseover
    tagSphere._on(self.$el, 'mouseover', () => {
      self.active = true
    })
    // mouseout
    tagSphere._on(self.$el, 'mouseout', () => {
      self.active = false
    })
    // mousemove
    tagSphere._on(self.keep ? window : self.$el, 'mousemove', (ev) => {
      ev = ev || window.event
  ...
  _next() {
    ...
    if (!self.keep && !self.active) {
      self.mouseX =
        Math.abs(self.mouseX - self.mouseX0) < 1 ? self.mouseX0 : (self.mouseX + self.mouseX0) / 2 // reset distance between the mouse and rolling center x axis
      self.mouseY =
        Math.abs(self.mouseY - self.mouseY0) < 1 ? self.mouseY0 : (self.mouseY + self.mouseY0) / 2 // reset distance between the mouse and rolling center y axis
    }

    const a =
      -(Math.min(Math.max(-self.mouseY, -self.size), self.size) / self.radius) * self.maxSpeed
    const b =
  ...
  destroy() {
    const self = this
    self.interval = null
    // clear in tagSphere.list
    const index = tagSphere.list.findIndex((e) => e.el === self.$el)
    if (index !== -1) tagSphere.list.splice(index, 1)
    // clear element
...
export default (els, texts, options) => {
  ...
  els.forEach((el) => {
    if (el) {
      instances.push(new tagSphere(el, texts, options))
    }
  })
